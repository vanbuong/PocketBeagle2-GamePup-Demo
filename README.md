...
> (`system-clock-direction-out` + `auxclk-fs-ratio`). BeaglePlay HDMI audio
> uses the same pattern with no gpio-gate clock.
>
> A common `-ENODEV: parse error` cause is a mistyped codec child name:
> it must be `simple-audio-card,codec` (or `codec` under a
> `simple-audio-card,dai-link`), **not** `sound-audio-card,codec`.
