...
> **Note:** `simple-audio-card` only supports one codec per McASP. Capture for
> the INMP441 needs audio-graph / a second card later. Playback uses
> `audio-graph-card` so the overlay does **not** need a base-DTB `mcasp2`
> symbol (a missing symbol makes U-Boot print `FDT_ERR_NOTFOUND`).
>
...
>
> If U-Boot reports `failed on fdt_overlay_apply(): FDT_ERR_NOTFOUND`, the
> overlay still references a label missing from the base DTB `/__symbols__`
> (often `&mcasp2`). This tree uses `target-path` for McASP2 instead. Check:
> `fdtget /boot/dtb/ti/k3-am6232-pocketbeagle2.dtb /__symbols__ mcasp2`.
...
# expect: max98357-playback-v5-audio-graph
...
sudo modprobe snd-soc-davinci-mcasp snd-soc-max98357a snd-soc-audio-graph-card
# re-trigger card bind if it probed before the codec/CPU DAIs existed:
echo sound-gamepup | sudo tee /sys/bus/platform/drivers/asoc-audio-graph-card/unbind 2>/dev/null
echo sound-gamepup | sudo tee /sys/bus/platform/drivers/asoc-audio-graph-card/bind 2>/dev/null
aplay -l
...
dmesg | grep -iE 'sound|mcasp|max98357|graph-card|2b20000'
ls /proc/device-tree/sound-gamepup/
